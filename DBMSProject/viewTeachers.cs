    ...
    public partial class viewTeachers : Form
    {
        ...
        private void getTeachers()
        {
            ...
            if (search == "%")
            {
                ...
                {
                    con.Open();
                    cmd = new SqlCommand("select UserId,Username,Email,DOB,Gender,Dept_name from EMS_User E,Department D where E.DeptId=D.DeptId AND RoleId=2 ", con);
            ...
            else
            {
                ...
                {
                    con.Open();
                    cmd = new SqlCommand("select UserId,Username,Email,DOB,Gender,Dept_name from EMS_User E,Department D where E.DeptId=D.DeptId AND RoleId=2 AND Username like '"+search+"' ", con);
                    SqlDataAdapter da = new SqlDataAdapter(cmd);
                    DataTable dt = new DataTable();
                    da.Fill(dt);
                    dataGridView1.DataSource = dt;
